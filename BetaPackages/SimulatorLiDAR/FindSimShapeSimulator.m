...
cd 'C:\Users\Dario\Desktop\Test_Simulateur\Disp'
files = dir('*.txt');
tic
...
parfor i=1:length(files)
        pct(i) = AffinTransform(pc(i),TransformMatrix([0,0,0])); %[Deg2Rad(90-5),0,0]    [Deg2Rad(90-35),0,0]
...
if isempty(ReconSphere.Colors) && isempty(ReconSphere.Intensities)
    ...
    ReconSphere.plot3(false,'MarkerSize',1,'Marker','o','LineStyle','none','Color',[0 0 1]);
...
for k= 1:Spheres  
    ...
    %sph = GenSphere(5,25);
    sph = GenParallelepiped(100,50,20,1000);
    ...
    for j= 1:1:filesPC
        %%% Create the sphere that will be use to fit to the scan.
        %sph = GenSphere(5,25);
        sph = GenParallelepiped(100,50,20,1000); %working with a half sphere improves things much better.
        sph.Name = 'Test Sphere';
        sph.copyTrue2MeasPos;
        if isempty(TMTot)
...
close(barre);
%%
Displacement(1:3,:) = Results(1:3,1,:);

scatter3(Displacement(1,:),Displacement(2,:),Displacement(3,:));
