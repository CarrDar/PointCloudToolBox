...
Post_event=PointCloud;
Post_event=ImportPointCloudFromASCII('C:\MATLAB\Matlab_pctb\_Data\_RockfallMonitoring\Synthetic_test_post_event.txt','Post',{'P'});
Post_event.addNoise('OutlierProb',0.001, 'GaussSmear', [0.01 10],'DropOutProb', 0.0);
Post_event.TLSPos=[0;0;1000];

%Pre_event=PointCloud;
Pre_event=ImportPointCloudFromASCII('C:\MATLAB\Matlab_pctb\_Data\_RockfallMonitoring\Synthetic_test_pre_event.txt','Pre',{'P'});
Pre_event.addNoise('OutlierProb',0.001, 'GaussSmear', [0.01 10],'DropOutProb', 0.0);
Pre_event.TLSPos=[0;0;1000];
tic
[PCRockfall,T1] = RockfallExtract(Pre_event,Post_event,[],[],'P2P',0); %%#ok<MSNU>
figure;
[Rockfalls_events,Eps] = RockfallSegment(PCRockfall,100,15,1); %#o%#ok<MSNU> k<NASGU>
figure;
[BlocksVolume, SS] = RockfallVolume(Rockfalls_events,[],1);
toc